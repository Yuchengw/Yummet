...
public class RedisProvider extends AbstractBaseRedisDao<String, EntityObject>
	...
	private static final RedisProvider service=  (RedisProvider)  new ClassPathXmlApplicationContext(
			"classpath:spring.xml").getBean("redisProvider");
	...
	public EntityObject get(final EntityObject record) {
		EntityObject result = redisTemplate
				.execute(new RedisCallback<EntityObject>() {
					public EntityObject doInRedis(RedisConnection connection)
							throws DataAccessException {
						RedisSerializer<String> serializer = getRedisStringSerializer();
						byte[] key = serializer.serialize(record.getCacheKey());
